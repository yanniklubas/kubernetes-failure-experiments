...
setup_autoscaling_realistic() {
    ...
    if gcloud container node-pools list --cluster="$CLUSTER" --zone="$ZONE" --format="value(name)" | grep -Fx "$LARGE_POOL"; then
        log_command gcloud container node-pools delete "$LARGE_POOL" --cluster="$CLUSTER" --zone="$ZONE" --quiet
    fi
...
start_robot_shop() {
    ...
    for pvc in $(kubectl get pvc -n default -o jsonpath='{.items[*].metadata.name}'); do
        kubectl patch pvc "$pvc" -n default -p ''\''{"metadata":{"finalizers":[]}}'\''' --type=merge || true
    done
...
start_loadgenerator() {
    ...
    if [[ "$EXPERIMENT_MODE" == "region" ]]; then
        ...
        for region in "${REGIONS[@]}"; do
            ...
            local ip_and_port
            ip_and_port=$(get_web_ip_and_port_region "$region")

            gcloud compute ssh --zone "$zone" "$node" --command="cd kubernetes-failure-experiments; ./setup-loadgenerator.sh $ip_and_port"
        done
        ip_and_port=$(get_web_ip_and_port_region "us")
        ./setup-loadgenerator.sh "$ip_and_port"
